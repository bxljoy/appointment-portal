...
export const provisionAppRole = async (client: PoolClient, password: string): Promise<void> => {
  ...
  await client.query(`
    ...
    REVOKE ALL ON ALL TABLES IN SCHEMA public FROM portal_app;
    REVOKE ALL ON ALL SEQUENCES IN SCHEMA public FROM portal_app;
    -- Table revocation alone does not revoke an older column-level grant.
    REVOKE ALL (id, cognito_sub, display_name, role, created_at) ON users FROM portal_app;
    REVOKE ALL (user_id, biography, specialty, timezone, created_at) ON clinician_profiles FROM portal_app;
    REVOKE ALL (id, clinician_id, start_at, end_at, status, created_at) ON availability_slots FROM portal_app;
    REVOKE ALL (id, slot_id, patient_id, status, cancelled_at, cancelled_by, created_at) ON appointments FROM portal_app;
