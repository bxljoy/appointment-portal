...
import { BookInputSchema, type BookInput, type Slot } from '@portal/contracts';
...
import { BookingReconciliationError, useBookAppointment } from './queries';

export function BookingForm({ slots, timezone, formatSlot }: { slots: Slot[]; timezone: string; formatSlot(slot: Slot): string }) {
  const form = useForm<BookInput>({ resolver: zodResolver(BookInputSchema), defaultValues: { slotId: slots[0]?.id } });
  ...
  const submit = async (values: BookInput) => {
    setMessage(undefined);
    setReconciliationRequired(false);
    try {
      const outcome = await booking.mutateAsync(values);
      setMessage(outcome === 'confirmed' ? 'Appointment confirmed' : 'We did not find a booking for this time. Availability was refreshed before you try again.');
    } catch (error) {
      if (error instanceof ApiClientError && error.status === 409) setMessage('This slot was just booked. Please choose another.');
      else if (error instanceof BookingReconciliationError) {
  ...
  return <form onSubmit={form.handleSubmit(submit)} aria-label="Book an appointment" className="space-y-5">
    ...
    {reconciliationRequired && <p className="text-sm"><Link className="text-primary underline-offset-4 hover:underline" to="/appointments">Open your appointments</Link> before attempting another booking.</p>}
    <Button type="submit" aria-label="Book appointment" disabled={booking.isPending || reconciliationRequired || !form.watch('slotId')}>{booking.isPending ? 'Booking appointment…' : 'Book appointment'}</Button>
  </form>;
}
