...
function AppointmentRow({ appointment, viewerTimezone, onCancelled }: { appointment: Appointment; viewerTimezone: string; onCancelled(): void }) {
  const canCancel = appointment.status === 'booked' && !started(appointment);
  return <li className="flex flex-col gap-3 rounded-md border bg-surface p-5 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="font-semibold">{appointment.clinicianDisplayName}</h3><p className="text-sm text-muted-foreground">{showTime(appointment.startAt, viewerTimezone)}</p><p className="mt-1 text-sm">{appointment.status === 'cancelled' ? 'Cancelled' : started(appointment) ? 'Started appointment' : 'Upcoming appointment'}</p></div>{canCancel && <CancelDialog appointment={appointment} onCancelled={onCancelled} />}</li>;
}
...
export function PatientAppointmentsPage() {
  ...
  const moveFocusToHistory = () => { historyHeading.current?.focus(); };
