...
export function ClinicianDirectoryPage() {
  ...
  const directory = useClinicians();
  const specialty = searchParams.get('specialty') ?? '';
  const clinicians = directory.data?.items.filter((clinician) => !specialty || clinician.specialty === specialty) ?? [];
  const specialties = [...new Set(directory.data?.items.map((clinician) => clinician.specialty) ?? [])].sort();
  ...
  return <section aria-labelledby="directory-title" className="space-y-8">
    ...
    {directory.data && clinicians.length === 0 && <div role="status" className="rounded-md border bg-surface p-6"><h2>No clinicians match this filter.</h2><p className="mt-2 text-muted-foreground">Try another specialty or check back later.</p></div>}
