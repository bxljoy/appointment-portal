...
import { authorityFromManifest } from './aws-authority.js';
...
const evidenceSchema = z.strictObject({
  ...
  issuer: z.url().startsWith('https://'), clientId: z.string().min(1), userPoolId: z.string().min(1), cognitoDomain: z.url().startsWith('https://'),
...
const identity = (manifest: DeploymentManifest) => {
  ...
  const frontendUrl = authority.frontendUrl;
  ...
  return { account: authority.account, region: authority.region, commit: manifest.sourceCommit, frontendUrl, distributionId,
    issuer: authority.issuer, clientId: authority.clientId, userPoolId: authority.userPoolId, cognitoDomain: authority.cognitoDomain };
};
...
export function manualRegistrationCheck(manifest: DeploymentManifest, evidence: ManualRegistrationEvidence | undefined, now = new Date()) {
  ...
  return { name, status: 'manual-passed' as const,
    detail: 'A human confirmed registration, email verification, initial patient role, sign-in, sign-out, and password recovery for the controlled test account.' };
}
